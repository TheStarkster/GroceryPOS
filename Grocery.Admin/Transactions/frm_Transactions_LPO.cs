...
{
    public partial class frm_Transactions_LPO : Form
    {
        string FormName = "LPO";
        string msg = "";

        #region "Form Level Activity" 
        public frm_Transactions_LPO()
        {
            InitializeComponent();
            forminitialization();
        }
        ...
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = LPO.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cbox_Currency, "CURRENCY");
                GolobalItems.ComboBoxBind(cbox_CostCenter, "BRANCH");
            }
            catch (Exception ex)
            {
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_InvoiceID.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cbox_Currency.SelectedIndex = 0; cbox_CostCenter.SelectedIndex = 0;
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_CurrencyRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_InvoiceID.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_LPO_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_InvoiceID.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = LPO.Get("SINGLE", txt_InvoiceID.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_InvoiceID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    txtSuppid.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custid"]);
                    txt_SupplierName.txtList.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["suppname"]);
                    txt_SupplierRef.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custRefe"]);
                    dtp_DateOfInvoice.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);

                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_PaymentMode_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_PaymentMode_Credit.Checked = true;

                    txttotalAmount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txtdiscAmount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txtNetAmount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);
                    txt_Desc.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["Description"]);
                    cbox_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurrency"]);
                    txt_CurrencyRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);
                    dtpdeldate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["deldate"]);
                    txtdelterms.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["delterms"]);
                    txtpriceterms.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["priceterms"]);
                    txtpaymentterms.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["paymentterms"]);
                    txtTotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["LpoType"]) == "Branches")
                        rb_LPO4Branches.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["LpoType"]) == "Supplier")
                        rb_LPO4Supplier.Checked = true;
                }
                ...
                foreach (DataRow row in ds.Tables[1].Rows)
                {
                    ...
                    dgv_item.Rows[i].Cells["UnitPriceAED"].Value = row["UnitPriceAED"];
                    dgv_item.Rows[i].Cells["UnitPriceAED"].Value = row["UnitPriceAED"];
        ...
        private void Save()
        {
            try
            {
                LPO_Master objMaster = new LPO_Master();
                List<LPO_Details> listDetail = new List<LPO_Details>();

                objMaster.invoiceId = txt_InvoiceID.Text;
                objMaster.custId = txtSuppid.Text;
                objMaster.salesDate = dtp_DateOfInvoice.Value;

                if (rb_PaymentMode_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_PaymentMode_Credit.Checked)
                    objMaster.salesType = "Credit";

                objMaster.totalAmount = GolobalItems.NullToNumber(txttotalAmount.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txtdiscAmount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txtNetAmount.Text);
                objMaster.Description = txt_Desc.Text;
                objMaster.custRefe = txt_SupplierRef.Text;
                objMaster.purcurrency = cbox_Currency.Text;
                objMaster.currate = txt_CurrencyRate.Text;
                objMaster.deldate = dtpdeldate.Value;
                objMaster.delterms = txtdelterms.Text;
                objMaster.priceterms = txtpriceterms.Text;
                objMaster.paymentterms = txtpaymentterms.Text;
                objMaster.TotalVAT = GolobalItems.NullToNumber(txtTotalVAT.Text);
                ...
                if (rb_LPO4Branches.Checked)
                    objMaster.LpoType = "Branches";
                else if (rb_LPO4Supplier.Checked)
                    objMaster.LpoType = "Supplier";

                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    LPO_Details objLine = new LPO_Details();
                    objLine.invoiceId = txt_InvoiceID.Text;
                ...
                msg = LPO.Set(objMaster, listDetail);
        ...
        {
            try { txt_CurrencyRate.Text = (string)(cbox_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString(); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
            }
        ...
        private void txtSuppid_MouseClick(object sender, MouseEventArgs e)
        {
            PopulatePopGrid("SUPPLIER_LIST");
        }
