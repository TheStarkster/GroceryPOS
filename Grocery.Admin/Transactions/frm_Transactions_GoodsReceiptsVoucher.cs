...
{
    public partial class frm_Transactions_GoodsReceiptsVoucher : Form
    {
        string FormName = "GOOD_RECEIPT_VOUCHER";
        string msg = "";

        #region "Form Level Activity" 

        public frm_Transactions_GoodsReceiptsVoucher()
        {
            InitializeComponent();
            forminitialization();
        }
        ...
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = GoodsReceiptsVoucher.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cob_GoodsReceiptVoucher_Currency, "CURRENCY");
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_GoodsReceiptVoucher_InvoiceID.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cob_GoodsReceiptVoucher_Currency.SelectedIndex = 0; cob_CostCenter.SelectedIndex = 0;
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_GoodsReceiptVoucher_CurRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_GoodsReceiptVoucher_InvoiceID.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_GoodsReceiptsVoucher_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_GoodsReceiptVoucher_InvoiceID.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void PopupForm_DateSet()
        {
            try
            {
                ...
                {
                    txt_GoodsReceiptVoucher_SupplierName.txtList.Text = GolobalItems.glbListItem.Items[0].ToString();
        ...
        private void Calculation_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_GoodsReceiptVoucher_TotalAmt.Text = TotalAmount.ToString();
                txt_GoodsReceiptVoucher_NetAmt.Text = TotalAmount.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = GoodsReceiptsVoucher.Get("SINGLE", txt_GoodsReceiptVoucher_InvoiceID.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_GoodsReceiptVoucher_InvoiceID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    txtSuppid.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custid"]);
                    txt_GoodsReceiptVoucher_SupplierName.txtList.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["suppname"]);

                    dt_GoodsReceiptVoucher_GRVDate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_GoodsReceiptVoucher_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_GoodsReceiptVoucher_Credit.Checked = true;
                   

                    txt_GoodsReceiptVoucher_TotalAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txt_GoodsReceiptVoucher_Discount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txt_GoodsReceiptVoucher_NetAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);

                    cob_GoodsReceiptVoucher_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurrency"]);
                    txt_GoodsReceiptVoucher_CurRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);

                    //txt_GoodsReceiptVoucher_TotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
        ...
        private void Save()
        {
            try
            {
                grv_master objMaster = new grv_master();
                List<grv_details> listDetail = new List<grv_details>();

                objMaster.invoiceId = txt_GoodsReceiptVoucher_InvoiceID.Text;
                objMaster.custId = txtSuppid.Text;
                objMaster.salesDate = dt_GoodsReceiptVoucher_GRVDate.Value;

                if (rb_GoodsReceiptVoucher_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_GoodsReceiptVoucher_Credit.Checked)
                    objMaster.salesType = "Credit";
                

                objMaster.totalAmount = GolobalItems.NullToNumber(txt_GoodsReceiptVoucher_TotalAmt.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txt_GoodsReceiptVoucher_Discount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txt_GoodsReceiptVoucher_NetAmt.Text);
                objMaster.Description = txt_GoodsReceiptVoucher_Desc.Text;
                objMaster.purcurrency = cob_GoodsReceiptVoucher_Currency.Text;
                objMaster.currate = txt_GoodsReceiptVoucher_CurRate.Text;

                objMaster.TotalVAT = GolobalItems.NullToNumber(txt_GoodsReceiptVoucher_TotalVAT.Text);
                ...
                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    grv_details objLine = new grv_details();
                    objLine.invoiceId = txt_GoodsReceiptVoucher_InvoiceID.Text;
                ...
                msg = GoodsReceiptsVoucher.Set(objMaster, listDetail);
        ...
        private void cob_GoodsReceiptVoucher_Currency_SelectedIndexChanged(object sender, EventArgs e)
        {
            try { txt_GoodsReceiptVoucher_CurRate.Text = (string)(cob_GoodsReceiptVoucher_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString(); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
            }
