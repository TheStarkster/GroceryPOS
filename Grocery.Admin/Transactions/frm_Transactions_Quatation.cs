...
{
    public partial class frm_Transactions_Quatation : Form
    {
        string FormName = "QUOTATION";
        string msg = "";
        #region "Form Level Activity" 

        public frm_Transactions_Quatation()
        {
            InitializeComponent();
            forminitialization();
        }
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = Quatation.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cob_Quotation_Currency, "CURRENCY");
                GolobalItems.ComboBoxBind(cob_Quotation_CostCenter, "BRANCH");
                cob_Quotation_CostCenter.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_Quotation_InvoiceNo.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cob_Quotation_Currency.SelectedIndex = 0; cob_Quotation_CostCenter.SelectedIndex = 0;
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_Quotation_CurRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_Quotation_InvoiceNo.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_Quatation_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_Quotation_InvoiceNo.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void PopupForm_DateSet()
        {
            try
            {
                if (PopupForm_Flag == "CUSTOMER_LIST")
                {
                    ...
                    txt_Quotation_CustID.Text = GolobalItems.glbListItem.Items[1].ToString();
                    txt_Quotation_CustTRN.Text = GolobalItems.glbListItem.Items[2].ToString();
                }
        ...
        private void Calculation_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_Quotation_TotalAmt.Text = TotalAmount.ToString();
                txt_Quotation_NetAmt.Text = TotalAmount.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = Quatation.Get("SINGLE", txt_Quotation_InvoiceNo.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_Quotation_InvoiceNo.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    txt_Quotation_CustID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custid"]);
                    txt_CustName.txtList.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custName"]);

                    dt_Quotation_Date.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_Quotation_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_Quotation_Credit.Checked = true;
                  
                    txt_Quotation_TotalAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txt_Quotation_Discount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txt_Quotation_NetAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);

                    cob_Quotation_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurrency"]);
                    txt_Quotation_CurRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);

                    txt_Quotation_TotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
        ...
        private void Save()
        {
            try
            {
                quot_master objMaster = new quot_master();
                List<quot_details> listDetail = new List<quot_details>();

                objMaster.invoiceId = txt_Quotation_InvoiceNo.Text;
                objMaster.custId = txt_Quotation_CustID.Text;

                if (rb_Quotation_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_Quotation_Credit.Checked)
                    objMaster.salesType = "Credit";
                

                objMaster.totalAmount = GolobalItems.NullToNumber(txt_Quotation_TotalAmt.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txt_Quotation_Discount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txt_Quotation_NetAmt.Text);
                objMaster.Description = txt_Quotation_Desc.Text;
                objMaster.purcurrency = cob_Quotation_Currency.Text;
                objMaster.currate = txt_Quotation_CurRate.Text;
                //objMaster.deldate = dt_Quotation_DueDate.Value;

                objMaster.TotalVAT = GolobalItems.NullToNumber(txt_Quotation_TotalVAT.Text);
                ...
                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    quot_details objLine = new quot_details();
                    objLine.invoiceId = txt_Quotation_InvoiceNo.Text;
                ...
                msg = Quatation.Set(objMaster, listDetail);
        ...
        {
            try { txt_Quotation_CurRate.Text = (string)(cob_Quotation_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString(); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
            }
