...
{
    public partial class frm_Transactions_PerformaInvoice : Form
    {
        string FormName = "PROFORMA_INVOICE";
        string msg = "";

        #region "Form Level Activity" 

        public frm_Transactions_PerformaInvoice()
        {
            InitializeComponent();
            forminitialization();
        }
        ...
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = PerformaInvoice.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cob_PerformaInvoice_Currency, "CURRENCY");
                GolobalItems.ComboBoxBind(cob_PerformaInvoice_CostCenter, "BRANCH");
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_PerformaInvoice_InvoiceID.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cob_PerformaInvoice_Currency.SelectedIndex = 0;
                    cob_PerformaInvoice_CostCenter.SelectedIndex = cob_PerformaInvoice_CostCenter.FindString(GolobalItems.BranchName);
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_PerformaInvoice_CurRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_PerformaInvoice_InvoiceID.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_PerformaInvoice_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_PerformaInvoice_InvoiceID.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void PopupForm_DateSet()
        {
            try
            {
                if (PopupForm_Flag == "CUSTOMER_LIST")
                {
                    ...
                    txt_PerformaInvoice_CustID.Text = GolobalItems.glbListItem.Items[1].ToString();
        ...
        private void dgv_item_EditingControlShowing(object sender, DataGridViewEditingControlShowingEventArgs e)
        {
            try
            {
                ...
                if (cell.OwningColumn == dgv_item.Columns["salesQty"]  || cell.OwningColumn == dgv_item.Columns["unitPrice"] || cell.OwningColumn == dgv_item.Columns["BonusQty"] )
                {
        ...
        private void Calculation_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_PerformaInvoice_SalesAmt.Text = TotalAmount.ToString();
                txt_PerformaInvoice_TotOutstanding.Text = TotalAmount.ToString();
                txt_PerformaInvoice_NetAmt.Text = TotalAmount.ToString();
                txt_PerformaInvoice_TotalQty.Text = TotalQty.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = PerformaInvoice.Get("SINGLE", txt_PerformaInvoice_InvoiceID.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_PerformaInvoice_InvoiceID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    txt_PerformaInvoice_CustID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custid"]);
                    txt_CustomerName.txtList.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custName"]);

                    dt_PerformaInvoice_SalesDate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_PerformaInvoice_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_PerformaInvoice_Credit.Checked = true;


                    txt_PerformaInvoice_SalesAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txt_PerformaInvoice_Discount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txt_PerformaInvoice_NetAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);

                    cob_PerformaInvoice_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurency"]);
                    txt_PerformaInvoice_CurRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);

                    txt_PerformaInvoice_TotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
        ...
        private void Save()
        {
            try
            {
                proforma_master objMaster = new proforma_master();
                List<proforma_details> listDetail = new List<proforma_details>();

                objMaster.invoiceId = txt_PerformaInvoice_InvoiceID.Text;
                objMaster.custId = txt_PerformaInvoice_CustID.Text;
                objMaster.salesDate = dt_PerformaInvoice_SalesDate.Value;

                if (rb_PerformaInvoice_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_PerformaInvoice_Credit.Checked)
                    objMaster.salesType = "Credit";


                objMaster.totalAmount = GolobalItems.NullToNumber(txt_PerformaInvoice_SalesAmt.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txt_PerformaInvoice_Discount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txt_PerformaInvoice_NetAmt.Text);
                objMaster.Description = txt_PerformaInvoice_Desc.Text;
                objMaster.purcurency = cob_PerformaInvoice_Currency.Text;
                objMaster.currate = txt_PerformaInvoice_CurRate.Text;
                //objMaster.deldate = dt_PerformaInvoice_DueDate.Value;

                objMaster.TotalVAT = GolobalItems.NullToNumber(txt_PerformaInvoice_TotalVAT.Text);
                ...
                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    proforma_details objLine = new proforma_details();
                    objLine.invoiceId = txt_PerformaInvoice_InvoiceID.Text;
                ...
                msg = PerformaInvoice.Set(objMaster, listDetail);
        ...
        private void cob_PerformaInvoice_Currency_SelectedIndexChanged(object sender, EventArgs e)
        {
            try
            {
                if (cob_PerformaInvoice_Currency.SelectedValue != null)
                {
                    txt_PerformaInvoice_CurRate.Text = (string)(cob_PerformaInvoice_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString();
                }
            }
            catch (Exception ex)
            {
        ...
        private void dt_PerformaInvoice_DueDate_ValueChanged(object sender, EventArgs e)
        {
            try
            {
                cob_PerformaInvoice_NoOfDays.Text = "";
                cob_PerformaInvoice_NoOfDays.Text = dt_PerformaInvoice_DueDate.Value.Subtract(DateTime.Now).Days.ToString();
                txt_PerformaInvoice_TermsConditions.Text = "PAYMENT TERM :- PDC " + dt_PerformaInvoice_DueDate.Value.Subtract(DateTime.Now).Days.ToString() + " Days from Bill Date";
        ...
        private void rb_PerformaInvoice_CreditCard_CheckedChanged(object sender, EventArgs e)
        {
            if (rb_PerformaInvoice_CreditCard.Checked) { cob_PerformaInvoice_PayMode.Text = "CREDIT CARD"; }
        }
    }
}
