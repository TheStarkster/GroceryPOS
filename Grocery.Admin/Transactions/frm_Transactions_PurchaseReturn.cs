...
{
    public partial class frm_Transactions_PurchaseReturn : Form
    {
        string FormName = "PURCHASE_RETURN";
        string msg = "";

        #region "Form Level Activity" 
        public frm_Transactions_PurchaseReturn()
        {
            InitializeComponent();
            forminitialization();
        }
        ...
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = PurchaseReturn.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cob_PurchaseReturn_Currency, "CURRENCY");
                GolobalItems.ComboBoxBind(cob_PurchaseReturn_CostCenter, "BRANCH");
            }
            catch (Exception ex)
            {
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_PurchaseReturn_PRNo.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cob_PurchaseReturn_Currency.SelectedIndex = 0; cob_PurchaseReturn_CostCenter.SelectedIndex = 0;
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_PurchaseReturn_CurRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_PurchaseReturn_PRNo.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_PurchaseReturn_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_PurchaseReturn_PRNo.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void Calculation_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_PurchaseReturn_TotalAmt.Text = TotalAmount.ToString();
                txt_PurchaseReturn_NetAmt.Text = TotalAmount.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = PurchaseReturn.Get("SINGLE", txt_PurchaseReturn_PRNo.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_PurchaseReturn_PRNo.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    ...
                    dt_PurchaseReturn_PRDate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_PurchaseReturn_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_PurchaseReturn_Credit.Checked = true;
                   

                    txt_PurchaseReturn_TotalAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txt_PurchaseReturn_Discount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txt_PurchaseReturn_NetAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);

                    cob_PurchaseReturn_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurrency"]);
                    txt_PurchaseReturn_CurRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);

                    txt_PurchaseReturn_TotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
        ...
        private void Save()
        {
            try
            {
                purchasereturn_master objMaster = new purchasereturn_master();
                List<purchasereturn_details> listDetail = new List<purchasereturn_details>();

                objMaster.invoiceId = txt_PurchaseReturn_PRNo.Text;
                objMaster.custId = txtSuppid.Text;
                objMaster.salesDate = dt_PurchaseReturn_PRDate.Value;

                if (rb_PurchaseReturn_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_PurchaseReturn_Credit.Checked)
                    objMaster.salesType = "Credit";
                

                objMaster.totalAmount = GolobalItems.NullToNumber(txt_PurchaseReturn_TotalAmt.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txt_PurchaseReturn_Discount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txt_PurchaseReturn_NetAmt.Text);
                objMaster.Description = txt_PurchaseReturn_Desc.Text;
                objMaster.purcurrency = cob_PurchaseReturn_Currency.Text;
                objMaster.currate = txt_PurchaseReturn_CurRate.Text;
                //objMaster.deldate = dt_PurchaseReturn_DueDate.Value;

                objMaster.TotalVAT = GolobalItems.NullToNumber(txt_PurchaseReturn_TotalVAT.Text);
                ...
                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    purchasereturn_details objLine = new purchasereturn_details();
                    objLine.invoiceId = txt_PurchaseReturn_PRNo.Text;
                ...
                msg = PurchaseReturn.Set(objMaster, listDetail);
        ...
        private void cob_PurchaseReturn_Currency_SelectedIndexChanged(object sender, EventArgs e)
        {
            try { txt_PurchaseReturn_CurRate.Text = (string)(cob_PurchaseReturn_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString(); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
            }
