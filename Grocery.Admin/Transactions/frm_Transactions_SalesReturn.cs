...
{
    public partial class frm_Transactions_SalesReturn : Form
    {
        string FormName = "SALES_RETURN";
        string msg = "";

        #region "Form Level Activity" 

        public frm_Transactions_SalesReturn()
        {
            InitializeComponent();
            forminitialization();
        }
        ...
        private void forminitialization()
        {
            ...
            try
            {
                ...
                DataTabel_FirstPreNextLast = SalesReturn.Get("MASTER", "").Tables[0];
                GolobalItems.ComboBoxBind(cob_SalesReturn_Currency, "CURRENCY");
                GolobalItems.ComboBoxBind(cob_SalesReturn_CostCenter, "BRANCH");
                cob_SalesReturn_CostCenter.SelectedIndex = 0;
            }
            catch (Exception ex)
            {
        ...
        private void btnTransaction_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (mode == "Add")
                {
                    ...
                    txt_SalesReturn_InvoiceNo.Text = CommonFunction.GenerateNoSeries(GolobalItems.BranchCode, FormName);
                    cob_SalesReturn_Currency.SelectedIndex = 0; cob_SalesReturn_CostCenter.SelectedIndex = 0;
                    dgv_item.Rows.Add();
                    dgv_item.Rows[dgv_item.Rows.Count - 1].Cells["UnitPriceAED"].Value = txt_SalesReturn_CurRate.Text;
                }
                else if (mode == "Edit")
                {
                    Enable_Disable_Form(true);
                ...
                else if (mode == "View")
                {
                    ...
                    obj.ShowDialog();
                    txt_SalesReturn_InvoiceNo.Text = GolobalItems.UniqueId;
                    DataBindWithControl();
                    Enable_Disable_Form(false);
                }
                else if (mode == "Print")
        ...
        private void frm_Transactions_SalesReturn_Load(object sender, EventArgs e)
        {
        ...
        private void PopulateData()
        {
            try
            {
                if (DataTabel_FirstPreNextLast.Rows.Count > 0)
                {
                    ...
                    txt_SalesReturn_InvoiceNo.Text = row["invoiceId"].ToString();
                    DataBindWithControl();
                }
            }
            catch (Exception ex)
        ...
        private void PopupForm_DateSet()
        {
            try
            {
                ...
                {
                    txt_CustName.txtList.Text = GolobalItems.glbListItem.Items[0].ToString();
                    txt_SalesReturn_CustID.Text = GolobalItems.glbListItem.Items[1].ToString();
                    txt_SalesReturn_CustTRN.Text = GolobalItems.glbListItem.Items[2].ToString();
                }
        ...
        private void Calculation_TextChanged(object sender, EventArgs e)
        {
            try
            {
                ...
                txt_SalesReturn_TotalAmt.Text = TotalAmount.ToString();
                txt_SalesReturn_NetAmt.Text = TotalAmount.ToString();
            }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
        ...
        private void DataBindWithControl()
        {
            try
            {
                DataSet ds = SalesReturn.Get("SINGLE", txt_SalesReturn_InvoiceNo.Text);
                if (ds.Tables[0].Rows.Count == 1)
                {
                    txt_SalesReturn_InvoiceNo.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["invoiceId"]);
                    txt_SalesReturn_CustID.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custid"]);
                    txt_CustName.txtList.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["custName"]);

                    dt_SalesReturn_SRDate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesDate"]);
                    if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Cash")
                        rb_SalesReturn_Cash.Checked = true;
                    else if (GolobalItems.NullToString(ds.Tables[0].Rows[0]["salesType"]) == "Credit")
                        rb_SalesReturn_Credit.Checked = true;

                    txt_SalesReturn_TotalAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["totalAmount"]);
                    txt_SalesReturn_Discount.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["discAmount"]);
                    txt_SalesReturn_NetAmt.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["netAmount"]);

                    cob_SalesReturn_Currency.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["purcurrency"]);
                    txt_SalesReturn_CurRate.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["currate"]);

                    txt_SalesReturn_TotalVAT.Text = GolobalItems.NullToString(ds.Tables[0].Rows[0]["TotalVAT"]);
        ...
        private void Save()
        {
            try
            {
                salesreturn_master objMaster = new salesreturn_master();
                List<salesreturn_details> listDetail = new List<salesreturn_details>();

                objMaster.invoiceId = txt_SalesReturn_InvoiceNo.Text;
                objMaster.custId = txt_SalesReturn_CustID.Text;

                if (rb_SalesReturn_Cash.Checked)
                    objMaster.salesType = "Cash";
                else if (rb_SalesReturn_Credit.Checked)
                    objMaster.salesType = "Credit";


                objMaster.totalAmount = GolobalItems.NullToNumber(txt_SalesReturn_TotalAmt.Text);
                objMaster.discAmount = GolobalItems.NullToNumber(txt_SalesReturn_Discount.Text);
                objMaster.netAmount = GolobalItems.NullToNumber(txt_SalesReturn_NetAmt.Text);
                objMaster.Description = txt_SalesReturn_Description.Text;
                objMaster.purcurrency = cob_SalesReturn_Currency.Text;
                objMaster.currate = txt_SalesReturn_CurRate.Text;
                //objMaster.deldate = dt_SalesReturn_DueDate.Value;

                objMaster.TotalVAT = GolobalItems.NullToNumber(txt_SalesReturn_TotalVAT.Text);
                ...
                foreach (DataGridViewRow row in dgv_item.Rows)
                {
                    salesreturn_details objLine = new salesreturn_details();
                    objLine.invoiceId = txt_SalesReturn_InvoiceNo.Text;
                ...
                msg = SalesReturn.Set(objMaster, listDetail);
        ...
        {
            try { txt_SalesReturn_CurRate.Text = (string)(cob_SalesReturn_Currency.SelectedValue.ToString().Split('^')).GetValue(1).ToString(); }
            catch (Exception ex)
            {
                MessageBox.Show(ex.Message, GolobalItems.MessageCaption);
            }
